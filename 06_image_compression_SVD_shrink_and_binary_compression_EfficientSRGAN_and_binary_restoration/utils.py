...
def save_compression(compressed, filename):
    os.makedirs(filename, exist_ok=True)
    for c,name in zip(compressed,('U','S','VT')):
        name = os.path.join(filename, name+'.npz')
        np.savez_compressed(name, c)


def load_compression(filename):
    return [np.load(os.path.join(filename, f))['arr_0'] for f in os.listdir(filename)]
